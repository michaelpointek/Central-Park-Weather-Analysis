...
* Khali - Created a Flask route enabling interaction with the database, extracting average values of chosen parameters—like precipitation, snowfall, and both minimum and maximum temperatures—across the complete 154-year timeline. This route caters to requests encompassing one to four distinct seasons. Also created a visualization that employs the Flask route's response, displaying the 154-year data trend for any selected season(s).
